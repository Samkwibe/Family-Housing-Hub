...
export default function ParentChildrenManagement() {
    ...
    const loadChildAccounts = async () => {
        ...
        try {
            ...
            if (enrichedAccounts.length > 0 && !selectedChild) {
                setSelectedChild(enrichedAccounts[0]);
            }
        } catch (error) {
    ...
    const loadChildData = async (childId) => {
        try {
            ...
            const [tasks, chores, homework, wallet, behavior, rewards, messages, locationHistory] = await Promise.all([
                childTasksService.getChildTasks(childId, currentUser.uid),
                childChoresService.getChildChores(childId),
                childLearningService.getChildHomework(childId),
                childWalletService.getChildWallet(childId, currentUser.uid),
                childBehaviorService.getChildBehavior(childId),
                childRewardsService.getChildRewards(childId),
                messageService.getUserMessages(childId),
                getLocationHistory(childId)
            ]);
